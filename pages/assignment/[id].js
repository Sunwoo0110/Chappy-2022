...
export default function CodingPage(props) {
  ...
  const handleCheckPoint = async (code, action) => {
    console.log('[id]/checkpoint called');
    ...
    if (action === "test") {
      ...
      /* 검증 버튼 */
      setMode(3);
      console.log('setTC called at cpoint w/ m=3');
      await setTC(code);
      console.log('setHint called at cpoint w/ m=3');
      await setHint(code);
      // setRun(code);
  ...
  const setTC = async (code) => {
    console.log('setTC called');
    ...
    await axios.post(`/api/assignment/runcode`, {
      ...
      "assignmentId": assignment._id,
      "testNumber": num,
      // "testcaseNumber": validation.num,
    })
      .then((res) => {
        console.log('setTC Res');
  ...
  const setHint = async (code) => {
    console.log('setHINT called');
    const response = await axios.post('/api/assignment/hint', {
      //feedback api 완성되면 연결
      "code": code,
    })
    const cnt = 0;
    let payload = {
      content: Object(response.data),
      num: cnt,
    };
