                    ...
                    <div className={styles.studentinfo}>
                        ...
                        <div style={{width:"30%"}}>
                            <div className={styles.studentinfo_data}>
                                ...
                                <div className={styles.studentinfo_3}>{data.name}</div>
                            </div>
                            <div className={styles.studentinfo_data}>
                                <div className={styles.studentinfo_1}>학과</div>
                                <div className={styles.studentinfo_3}>{data.department}</div>
                            </div>
                            <div className={styles.studentinfo_data}>
                                <div className={styles.studentinfo_1}>학기수</div>
                                <div className={styles.studentinfo_3}>{data.semester}</div>
                            </div>
                        </div>
                        <div style={{width:"30%"}}>
                            <div className={styles.studentinfo_data}>
                                <div className={styles.studentinfo_1}>아이디</div>
                                <div className={styles.studentinfo_3}>{data.user_id}</div>
                            </div>
                            <div className={styles.studentinfo_data}>
                                <div className={styles.studentinfo_1}>연락처</div>
                                <div className={styles.studentinfo_3}>{data.cell_number}</div>
                            </div>
                            <div className={styles.studentinfo_data}>
                                <div className={styles.studentinfo_1}>이메일</div>
                                <div className={styles.studentinfo_3}>{data.email}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
function LearningInfo() {
    ...
    useEffect(async () => {
        if (status === "authenticated" && user_id != '') {
            const response = await fetch(`/api/aggregation/mypage/mylectures?user_id=${user_id}&open_semester=${semester}`, {
            method: 'GET',
            headers: {
                "Content-Type": 'application/json',
            },
            ...
            if (result?.success !== true) {
                ...
                console.log("asdf:",result.lectures)
    ...
    return (
        <div style={{width:"100%"}}>{
            Array.isArray(data)  ? //배열 로딩된 후에 리턴하도록
            <div className={styles.section_bg}>
                <div className={styles.section_title_bg}>
